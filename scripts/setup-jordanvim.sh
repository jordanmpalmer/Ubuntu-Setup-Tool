...
cd nvim || {
  echo "Failed to change directory to nvim"
  exit 1
}

# Switch to the specified branch
git switch 03-Work-Environment || {
...
sed -i "s/vn411f/$NEW_VALUE/g" "$INIT_FILE" || {
  echo "Failed to replace text in $INIT_FILE"
  exit 1
}
...
cd || {
  echo "Failed to return to home directory"
  exit 1
}
...
mv win32yank.exe "/mnt/c/Users/$NEW_VALUE/Documents/WSLPlugins/win32yank.exe" || {
